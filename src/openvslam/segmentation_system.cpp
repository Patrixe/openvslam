...
namespace openvslam {
    segmentation_system::segmentation_system(const std::shared_ptr<config> &cfg, const std::string &vocab_file_path,
        ...
        // tracking module
        tracker_ = new segmentation_tracking_module(cfg_, this, map_db_, bow_vocab_, bow_db_, seg_cfg);
        // mapping module
    ...
    segmentation_system::feed_monocular_frame(const cv::Mat &img, const cv::Mat &seg_img, const double timestamp,
        ...
        Mat44_t cam_pose_cw = (dynamic_cast<segmentation_tracking_module *>(this->tracker_))->track_monocular_image(img,
                                                                                                                    seg_img,
                                                                                                                    timestamp,
                                                                                                                    mask);
    ...
    Mat44_t segmentation_system::feed_stereo_frame(const cv::Mat &left_img, const cv::Mat &right_img,
        ...
        const Mat44_t cam_pose_cw = (dynamic_cast<segmentation_tracking_module *>(this->tracker_))->track_stereo_image(
                left_img, right_img, left_seg_img, right_seg_img, timestamp, mask);
