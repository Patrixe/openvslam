...
//                  ref_bearings_(ref_frm.undist_keypts_.get_slam_applicable_bearings()),
        ...
        bool base::find_most_plausible_pose(const eigen_alloc_vector<Mat33_t> &init_rots,
            ...
            for (unsigned int i = 0; i < num_hypothesis; ++i) {
                nums_valid_pts.at(i) = check_pose(init_rots.at(i), init_transes.at(i), is_inlier_match,
                                                  depth_is_positive,
        ...
        unsigned int base::check_pose(const Mat33_t &rot_ref_to_cur, const Vec3_t &trans_ref_to_cur,
            ...
            for (unsigned int i = 0; i < ref_cur_matches_.size(); ++i) {
                ...
                const Vec3_t pos_c_in_ref = solve::triangulator::triangulate(ref_bearing, cur_bearing, rot_ref_to_cur,
                                                                             trans_ref_to_cur);
                ...
                const auto is_valid_ref = ref_camera_->reproject_to_image(Mat33_t::Identity(), Vec3_t::Zero(),
                                                                          pos_c_in_ref,
                ...
                const auto is_valid_cur = cur_camera_->reproject_to_image(rot_ref_to_cur, trans_ref_to_cur,
                                                                          pos_c_in_ref,
        ...
        void base::apply_transformation_to_points(
            ...
            // camera centers
            const Vec3_t ref_cam_center = Vec3_t::Zero();
            const Mat33_t &rotationRefToCur = get_rotation_ref_to_cur();
            const Vec3_t &translationRefToCur = get_translation_ref_to_cur();

            assert(rotationRefToCur != Mat33_t::Zero());
            assert(translationRefToCur != Vec3_t::Zero());

            const Vec3_t cur_cam_center = -rotationRefToCur.transpose() * translationRefToCur;
            ...
            for (auto match : ref_current_matches) {
                ...
                const Vec3_t pos_c_in_cur = rotationRefToCur * pos_c_in_ref + translationRefToCur;
                if (!parallax_is_small && pos_c_in_cur(2) <= 0) {
                    continue;
                }
                ...
                Vec2_t reproj_in_cur;
                float x_right_in_cur;
                const auto is_valid_cur = cur_camera_->reproject_to_image(rotationRefToCur, translationRefToCur,
                                                                          pos_c_in_ref,
                                                                          reproj_in_cur, x_right_in_cur);
                if (!parallax_is_small && !is_valid_cur) {
                    continue;
