...
bool loop_detector::detect_loop_candidates() {
    ...
    spdlog::debug("Entering loop candidate detection");
    ...
    spdlog::debug("min score in covisibilities: {}", min_score);
    ...
    spdlog::debug("loop candidates: {}", init_loop_candidates.size());
    ...
    spdlog::debug("{} curr_cont_detected_keyframes", curr_cont_detected_keyfrm_sets.size());
    ...
    for (auto& curr : curr_cont_detected_keyfrm_sets) {
        ...
        spdlog::debug("{} loop candidates to validate", loop_candidates_to_validate_.size());
    }
...
bool loop_detector::select_loop_candidate_via_Sim3(const std::vector<data::keyframe*>& loop_candidates,
    ...
    spdlog::debug("Entering select_loop_candidate_via_Sim3");
    ...
    for (const auto &candidate : loop_candidates) {
        if (candidate->will_be_erased()) {
            spdlog::debug("Removed due to will be erased");
            continue;
        }
        ...
        spdlog::debug("Found {} matches", num_matches);
        // check the threshold
        if (num_matches < 20) {
            continue;
        }
        ...
        if (num_optimized_inliers < 20) {
            spdlog::debug("Removed due to not enough optimized inliers");
            continue;
        }
